...
export const FORMAS_PAGAMENTO = ["Pix", "Dinheiro", "Crediário", "Cartão Parcelado"] as const;
export type FormaPagamento = (typeof FORMAS_PAGAMENTO)[number];

// "Cartão de Crédito" é rótulo legado de registros antigos.
const CARTAO = ["cartão parcelado", "cartao parcelado", "cartão de crédito", "cartao de credito"];
const PIX = ["pix"];
const PARCELADAS = [...CARTAO, "crediário", "crediario"];
...
export function isCartao(forma?: string | null): boolean {
  return CARTAO.includes(norm(forma));
}
