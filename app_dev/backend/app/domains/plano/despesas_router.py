...
router = APIRouter(prefix="/despesas", tags=["Despesas"], dependencies=[Depends(get_user_id_from_token)])
...
def create_despesa(
    ...
    registrar_alteracao(
        db, user_id=user_id, entidade="despesa", entidade_id=despesa.id, acao="criou",
        resumo=f"Despesa {despesa.tipo_item} criada",
    )
    db.commit()
    db.refresh(despesa)
    return _to_out(despesa)
...
def update_despesa(
    ...
    antes = {k: getattr(despesa, k, None) for k in campos_alterados}
    ...
    _sincronizar_pagamento(db, despesa)
    depois = {k: getattr(despesa, k, None) for k in campos_alterados}
    registrar_alteracao(
        db, user_id=user_id, entidade="despesa", entidade_id=despesa_id, acao="editou",
...
def delete_despesa(
    ...
    if not despesa:
        raise HTTPException(status_code=404, detail="Despesa não encontrada")
    resumo = f"Despesa {despesa.tipo_item} apagada"
    db.delete(despesa)
    registrar_alteracao(
        db, user_id=user_id, entidade="despesa", entidade_id=despesa_id, acao="apagou", resumo=resumo,
    )
    db.commit()
